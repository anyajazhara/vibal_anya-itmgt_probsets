...
def shift_letter(letter, shift):
    ...
    *Note: the single underscore `_` is used to acknowledge the presence
    ...
    str
        ...
        a single space if the original letter was a space.2
    '''
    if letter == " ":
        return letter
    else:
...
def vigenere_cipher(message, key):
    ...
    if len(message) != len(key):
        ...
        for i in range(len(message)):
            extended_key = extended_key + key[i % len(key)]

    for i in range(len(message)):
        encrypted_message = encrypted_message + shift_by_letter(message[i], extended_key[i])
...
def scytale_cipher(message, shift):
    ...
    for i in range(len(message)):
        encoded_message = encoded_message + message[(i // shift) + (len(message) // shift) * (i % shift)]
